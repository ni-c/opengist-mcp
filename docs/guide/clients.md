...
behind a single HTTPS endpoint, so opengist-mcp can be reached from clients that cannot
...
preset is a opengist-mcp feature and belongs in `env` as shown.
...
Register `https://your-host/opengist/mcp` as a connector and you
