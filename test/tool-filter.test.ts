...
describe('selecting tools', () => {
  it('narrows tools/list to an allow list', async () => {
    expect(await toolNames({ allowTools: 'check_gist_like,get_gist' })).toEqual(
      ['check_gist_like', 'get_gist'].sort()
    );
  });
  ...
  it('subtracts the deny list from the allow list', async () => {
    expect(
      await toolNames({
        allowTools: 'check_gist_like,get_gist',
        denyTools: 'get_gist',
      })
    ).toEqual(['check_gist_like']);
  });
  ...
  it('lets the preset compose with extra names', async () => {
    expect(
      await toolNames({ allowTools: 'essential,check_gist_like' })
    ).toEqual([...ESSENTIAL_TOOLS, 'check_gist_like'].sort());
  });

  it('trims entries, ignores case and skips empty ones', async () => {
    expect(
      await toolNames({ allowTools: ' CHECK_GIST_LIKE ,, get_gist, ' })
    ).toEqual(['check_gist_like', 'get_gist'].sort());
  });
...
describe('a filtered-out tool', () => {
  it('cannot be called either, not merely hidden', async () => {
    ...
    const server = createServer(config({ allowTools: 'check_gist_like' }));
    const client = new Client({ name: 'test-client', version: '0.0.0' });
    const [clientTransport, serverTransport] =
      InMemoryTransport.createLinkedPair();
    await Promise.all([
    ...
    ).rejects.toThrow('Tool create_gist not found');
...
describe('refusing an unusable list', () => {
  it('rejects a name no tool has, and says which names exist', () => {
    ...
    expect(() =>
      createServer(config({ allowTools: 'check_gist_likz' }))
    ).toThrow(ToolFilterError);
    expect(() =>
      createServer(config({ allowTools: 'check_gist_likz' }))
    ).toThrow(/no tool matches "check_gist_likz".*check_gist_like/s);
  });
  ...
  it('applies the same rule to the deny list', () => {
    expect(() =>
      createServer(config({ denyTools: 'check_gist_likz' }))
    ).toThrow(/_DENY_TOOLS: no tool matches "check_gist_likz"/);
  });
...
describe('together with read-only mode', () => {
  ...
  it('names read-only as the reason, rather than calling the tool unknown', () => {
    ...
    let thrown: unknown;
    try {
      createServer(config({ ...readOnly, allowTools: 'create_gist' }));
    } catch (error) {
      thrown = error;
    }
  ...
  it('does not apply the write-tool rule to the deny list', async () => {
    ...
    expect(await toolNames({ ...readOnly, denyTools: 'create_gist' })).toEqual(
      [...READ_TOOLS].sort()
    );
  });
